...
int main(int argc, const char **argv) {
  ...
  if (argc > 1) {
    ...
    std::cout << "No custom map entered. Using default map \"map.osm\""
              << std::endl;
  ...
  while (!valid_values) {
    ...
    if (ValidInputPoint(start_x) && ValidInputPoint(start_y) &&
        ValidInputPoint(end_x) && ValidInputPoint(end_y)) {
      valid_values = true;
    } else {
      std::cout << "Some values you inserted were not valid. Please try again"
                << std::endl;
    }
  }
